...
inline void wout(const char *filename, const char *text, int r)
{

}

inline void wout1(const char *filename, const char *text, int r)
{
...
alsa_process_stream(cubeb_stream * stm)
{
  ...
  if (stm->stream_type == SND_PCM_STREAM_PLAYBACK && avail > 0) {
    ...
    if (wrote < 0) {
      avail = wrote; // the error handler below will recover us
      wout("ff.txt","snd_pcm_writei failed", wrote);
    } else {
      stream_buffer_decrement(stm, wrote);
  ...
  if (avail < 0) {
    wout("ff.txt","snd_pcm_recover failed", avail);
    pthread_mutex_unlock(&stm->mutex);
...
alsa_locked_pcm_close(snd_pcm_t * pcm)
{
  ...
  wout("ff.txt","destroy其实就是调用close", r);
  return r;
}
...
#undef LOAD
  ...
  r = alsa_locked_pcm_open(&dummy, CUBEB_ALSA_PCM_NAME, SND_PCM_STREAM_PLAYBACK,
                           NULL);
  wout("ff.txt","init测试打开default", r);
  if (r >= 0) {
    alsa_locked_pcm_close(dummy);
    wout("ff.txt","init测试关闭default", r);
  }
  ctx->is_pa = 0;
  pthread_mutex_lock(&cubeb_alsa_mutex);
                        ...
                        cubeb_state_callback state_callback, void * user_ptr)
{
  ...
  *stream = NULL;
  ...
  if (ctx->active_streams >= CUBEB_STREAM_MAX) {
    pthread_mutex_unlock(&ctx->mutex);
    wout("ff.txt","init single流太多",100);
    return CUBEB_ERROR;
  }
  ...
  r = alsa_locked_pcm_open(&stm->pcm, pcm_name, stm->stream_type,
                           ctx->local_config);
  char buffer[100];
  snprintf(buffer, sizeof(buffer), "init single pcm_name: %s, error: %s", pcm_name, snd_strerror(r));
  wout("ff.txt",buffer, r);
  if (r < 0) {
  ...
  if (alsa_register_stream(ctx, stm) != 0) {
    alsa_stream_destroy(stm);
    wout("ff.txt","alsa_register_stream err", 100);
    return CUBEB_ERROR;
  }
...
alsa_stream_destroy(cubeb_stream * stm)
{
  ...
  ctx = stm->context;
  wout("ff.txt","alsa_stream_destroy1",100);
  if (stm->other_stream) {
    wout("ff.txt","other_stream",100);
  ...
  if (stm->pcm) {
    ...
    wout("ff.txt","alsa_stream_destroy2",100);
  ...
  wout("ff.txt","alsa_stream_destroy3",100);
...
alsa_stream_stop(cubeb_stream * stm)
{
  ...
  wout("ff.txt","stop2",100);
  pthread_mutex_lock(&ctx->mutex);
  while (stm->state == PROCESSING) {
    wout("ff.txt","stop22",100);
