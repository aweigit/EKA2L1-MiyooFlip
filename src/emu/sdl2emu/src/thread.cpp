...
namespace eka2l1::sdl2 {
    ...
    static int graphics_driver_thread_initialization(emulator &state) {
        ...
        printf("graphics_driver_thread_initialization1\n");

        state.window = std::make_unique<drivers::emu_window_sdl2>();
        state.window->init("Hello there", eka2l1::vec2(0, 0),
            drivers::emu_window_flag_maximum_size);
        state.window->set_userdata(&state);

        printf("graphics_driver_thread_initialization2\n");
        ...
        printf("graphics_driver_thread_initialization3\n");

        drivers::emu_window_sdl2 *window = state.window.get();

        window->surface_change_hook = [&state](void *new_surface) {
            state.graphics_driver->update_surface(new_surface);
        };

        printf("graphics_driver_thread_initialization4\n");
        ...
        printf("graphics_driver_thread_initialization5\n");

        state.graphics_init_done.set();
        printf("graphics_driver_thread_initialization6\n");
        return 0;
    }
    ...
    static int graphics_driver_thread_deinitialization(emulator &state) {
        ...
        printf("graphics_driver_thread_deinitialization\n");
        return 0;
    }
    ...
    bool emulator_entry(emulator &state, uint32_t rotate) {
        state.stage_one();
        state.rotate=rotate;
    ...
    bool emulator_entry(emulator &state, const char*rom, const char*pkg) {
        state.stage_one();

        emu_Emulator_installDevice(pkg, rom, true);//安装设备，参数为固件包路径(ROM文件和RPKG文件) 
    ...
    bool emulator_entry_rescan_device(emulator &state) {
        state.stage_one();
