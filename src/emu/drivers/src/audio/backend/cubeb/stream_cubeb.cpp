...
//cubeb_stream *last_stream=nullptr;
...
namespace eka2l1::drivers {
    ...
    cubeb_audio_stream_base::cubeb_audio_stream_base(cubeb *context,  std::uint32_t sample_rate,
         std::uint8_t channels, data_callback callback, bool is_recording) 
        : stream_(nullptr)
        , callback_(callback)
        , idled_frames_(0)
        ...
        minimum_latency = 256;
#else
        minimum_latency = 100 * params.rate / 1000; // Firefox default
        ...
        //LOG_CRITICAL(DRIVER_AUD, "初始化参数: sample_rate:{}, channels:{}, is_recording:{}", sample_rate, channels, is_recording);

        //LOG_CRITICAL(DRIVER_AUD, "修改后参数: sample_rate:{}, channels:{}, is_recording:{}", params.rate, params.channels, is_recording);

        // if(last_stream)
        // {
        //     LOG_CRITICAL(DRIVER_AUD, "Trying to destroy last cubeb stream! ");
        //     cubeb_stream_stop(last_stream);//先关再销毁
        //     cubeb_stream_destroy(last_stream);//flip不知为何无法正常销毁，只能先手动销毁，不然无法创建新的流,跟alsa配置文件有关，设置配置文件路径即可
        //     last_stream=nullptr;
        // }
        

        const auto result = cubeb_stream_init(context, &stream_, "EKA2L1 StreamPeam",
            "default", is_recording ? &params : nullptr, "default", is_recording ? nullptr : &params, minimum_latency,
            data_callback_redirector, state_callback_redirector, this);


        if (result != CUBEB_OK) {
            LOG_CRITICAL(DRIVER_AUD, "Error trying to initialize cubeb stream! errcode:{}", result);
            //last_stream=nullptr;
            stream_=nullptr;
            return;
        }
    ...
    cubeb_audio_stream_base::~cubeb_audio_stream_base() {
        if (stream_) {
            LOG_INFO(DRIVER_AUD, "~cubeb_audio_stream_base");
